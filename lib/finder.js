...
class Finder extends react_1.Component {
    ...
    render() {
        ...
        const styleTaskBar = { left: margin, top: window.innerHeight - minHeight - margin, zIndex: zIndex };
        const nodes = [];
        this.handler.forEach(h => {
            nodes.push(h.node);
        });
        ...
        if (wrapperButtons) {
            ...
            wrapperButton = React.createElement(styled_1.Wrapper, { minHeight: minTaskBarHeight, minWidth: minTaskBarWidth, style: styleTaskBar, isActive: true, className: className }, this.renderButton());
        }
