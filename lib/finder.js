...
class Finder extends react_1.Component {
    ...
    constructor(props) {
        ...
        if (props.zIndexStart)
            this.zIndexStart = props.zIndexStart;
    ...
    add(node) {
        node.children;
        const id = node.wid ? node.wid : (0, uuid_1.v4)();
    ...
    create(wid, node) {
        ...
        const rn = (React.createElement(wm_1.WM, { key: "window_" + wid, wid: wid, title: node.title, isResizable: node.resizable, initialZIndex: initialZIndex, initialPosition: node.initialPosition, initialSize: node.initialSize, onClose: withTaskBar ? () => this.onClose(wid) : undefined, onMouseDown: () => this.onSelectWindow(wid), onMinimize: withTaskBar ? () => this.onMinimize(wid) : undefined, onFullscreen: withTaskBar ? () => this.onFullscreen(wid) : undefined, onMove: (state) => { this.onMove(wid, state); }, onResize: (state) => { this.onResize(wid, state); }, onMount: (wm) => this.onMount(wid, wm), onUnmount: (wm) => this.onUnmount(wid, wm), header: node.header, hprops: node.hProps }, node.children));
    ...
    render() {
        ...
        if (wrapperButtons) {
            ...
            wrapperButton = React.createElement(styled_1.Wrapper, { minHeight: minTaskBarHeight ? minTaskBarHeight : minHeight, minWidth: minTaskBarWidth, style: styleTaskBar, isActive: true, className: className }, this.renderButton());
        }
